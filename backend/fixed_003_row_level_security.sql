-- Fixed Row Level Security Migration
-- This version puts helper functions in public schema to avoid auth schema permission issues
...
-- Helper function to get user role (placed in public schema)
...
RETURNS BOOLEAN AS $$
BEGIN
    RETURN get_user_role() = ANY(roles);
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;
...
CREATE POLICY "Staff can view all documents"
    ON documents FOR SELECT
    USING (
        has_any_role(ARRAY['clerk', 'archivist', 'inspector', 'admin']::user_role[])
    );

-- Clerks and archivists can create documents
CREATE POLICY "Clerks can create documents"
    ON documents FOR INSERT
    WITH CHECK (
        has_any_role(ARRAY['clerk', 'archivist', 'admin']::user_role[])
        AND auth.uid() = uploader_user_id
    );

-- Only archivists and admins can update documents
CREATE POLICY "Archivists can update documents"
    ON documents FOR UPDATE
    USING (
        has_any_role(ARRAY['archivist', 'admin']::user_role[])
    )
    WITH CHECK (
        has_any_role(ARRAY['archivist', 'admin']::user_role[])
    );
...
CREATE POLICY "Files inherit document visibility"
    ...
    USING (
        EXISTS (
            ...
            AND (
                ...
                -- Or user is staff
                OR has_any_role(ARRAY['clerk', 'archivist', 'inspector', 'admin']::user_role[])
            )
        )
    );

-- Staff can create document files
CREATE POLICY "Staff can create document files"
...
CREATE POLICY "Archivists can update files"
    ON document_files FOR UPDATE
    USING (has_any_role(ARRAY['archivist', 'admin']::user_role[]));
...
CREATE POLICY "Inspectors can read audit logs"
    ON audit_logs FOR SELECT
    USING (
        has_any_role(ARRAY['inspector', 'admin']::user_role[])
    );
...
CREATE POLICY "Anyone can request document access"
...
CREATE POLICY "Archivists can process access requests"
...
-- Note: This may need to be modified based on your auth setup
...
CREATE POLICY "Citizens view own submissions"
    ON documents FOR SELECT
    USING (
        EXISTS (
...
BEGIN
    ...
    IF v_is_public THEN
        v_has_access := TRUE;
    ELSIF v_user_role IN ('clerk', 'archivist', 'inspector', 'admin') THEN
        v_has_access := TRUE;
    ELSIF EXISTS (
