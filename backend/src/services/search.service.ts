    ...
    async searchDocuments(
        ...
        `, {count: 'exact'})
            ...
            if (
                ...
                searchQuery.sort?.field === 'relevance'
            ) {
                // Use RPC function for ranked results
                const {data: ranked, error} = await supabaseAdmin
                    .rpc('ranked_document_search', {query_text: searchQuery.query})
                ...
                return {
                    ...
                    total: ranked?.length || 0,
                    facets: await this.calculateFacets(searchQuery),
